...
def img_crop(input_dir,out_dir,ty,cr):
    ...
    if ty=='no' and cr=='no':
        ...
        for i in range(0,l):
            ...
            txt_name=g.readlines()[i].replace('/n','').split(',')[0].replace('.jpg','.txt').replace('.jpg','.txt')
            g.close()
            
            g=open(cr+'/labels.txt')
            h=open(cr+'/'+txt_name,'a')
            h.writelines('0,0,0,0,0,0,0,0,'+g.readlines()[i].replace('/n','').split(',')[1].replace('"',''))
            h.writelines('/n')
            h.close()
            g.close()
        ...
        if os.path.isfile(img_path): 
            ...
            if '.jpg' in img_path:
                ...
                for i in range(0,le):
                    ...
                    outtxt=(file_name+','+'"'+h.readlines()[i].split(',')[-1].replace('/n','')+'"')
...
img_crop('C:/Users/User/Desktop/Batch_Code_Updated/One_click_training/Detection/working_directory/175_ml/new_laser/images/','C:/Users/User/Desktop/Batch_Code_Updated/One_click_training/Detection/working_directory/175_ml/new_laser/images/cropped/','no','no')
